...
import { useUpdateIncomeAccountMutation, type IncomeAccountItem } from "@entities/income-account";
import { useGetRegionalCentersQuery } from "@entities/regional-center";
import { useGetBranchesQuery } from "@entities/branch";
import type { EditIncomeAccountFormValues } from "../model/types";

const validate = (values: EditIncomeAccountFormValues) => {
  const errors: Partial<Record<keyof EditIncomeAccountFormValues, string>> = {};
...
interface EditIncomeAccountDrawerProps {
  item: IncomeAccountItem;
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function EditIncomeAccountDrawer({ item, isOpen, onClose, onSuccess }: EditIncomeAccountDrawerProps) {
  const [updateIncomeAccount, { isLoading }] = useUpdateIncomeAccountMutation();
  const { data: centers = [] } = useGetRegionalCentersQuery();

  const centerOptions = centers.map((c) => ({ value: c.id, label: c.name }));

  const initialValues: EditIncomeAccountFormValues = {
  ...
  const handleSubmit = async (values: EditIncomeAccountFormValues) => {
    try {
      await updateIncomeAccount({ id: item.id, data: values }).unwrap();
      onSuccess();
      onClose();
    } catch {
        ...
        <Drawer.Positioner>
          <Drawer.Content maxW="480px">
            <Drawer.Header fontSize="lg" fontWeight="semibold">
              <Drawer.Title>Редактировать счёт доходов</Drawer.Title>
            </Drawer.Header>
            <Drawer.CloseTrigger asChild position="absolute" top="3" insetEnd="3">
              <CloseButton size="sm" />
            </Drawer.CloseTrigger>
            <Form<EditIncomeAccountFormValues> onSubmit={handleSubmit} initialValues={initialValues} validate={validate}>
              {({ handleSubmit: submitForm, submitError }) => (
                <form onSubmit={submitForm} style={{ display: "contents" }}>
                  ...
                  <Drawer.Footer gap={3}>
                    ...
                    <Button type="submit" colorPalette="brand" loading={isLoading}>Сохранить</Button>
                  </Drawer.Footer>
                </form>
              )}
            </Form>
